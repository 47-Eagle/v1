 ...
 */
export function createEagleBridge(
  connection: Connection,
  wallet: Wallet,
  oftProgram: Program<EagleOftLayerzero>
): EagleBridge {
  const bridge = new EagleBridge(connection, wallet, oftProgram);
 ...
 *
 * ```typescript
 * import { createEagleBridge } from './eagle-bridge';
 *
 * // Setup
 * const bridge = createEagleBridge(connection, wallet, oftProgram);
 *
 * // Bridge EAGLE
 * const result = await bridge.bridgeEagle(
 *   1000, // amount
